...
namespace WebBanHangOnline.Areas.Admin.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Create(CreateAccountViewModel model)
        {
            ...
            {
                ModelState.AddModelError("Roles", "Bạn phải chọn ít nhất một quyền.");
            }
            if (ModelState.IsValid)
            {
        ...
        public async Task<ActionResult> DeleteAccount(string user, string id)
        {
            ...
            var item = await UserManager.FindByNameAsync(user); // Tìm người dùng theo tên

            if (item != null)
            {
                var rolesForUser = await UserManager.GetRolesAsync(item.Id); // Lấy roles của user
                if (rolesForUser != null && rolesForUser.Count > 0)
                {
                    foreach (var role in rolesForUser)
                    {
                        await UserManager.RemoveFromRoleAsync(item.Id, role); // Xóa role
                    }
                }

                var res = await UserManager.DeleteAsync(item); // Xóa user
                code = new { Success = res.Succeeded }; // Cập nhật kết quả
            }
            return Json(code); // Trả về kết quả
        }
